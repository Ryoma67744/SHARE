...
> Save 後、各切片パネル左上の **section 名ラベル** に `Section 1 · 50 μm/px` のように **Pixel pitch** が併記されます。これは share recipient の閲覧画面と Preview の cell title でも同じ書式で表示されます (受け手が解像度を確認できるようにするため)。
