...
> 各切片パネル左上の **section 名ラベル** には、提供者が Align 設定で μm/px を入れていれば「Section 1 · 50 μm/px」のように **Pixel pitch (μm/px)** が併記されます。x ≠ y の異方性ピクセルでは「50×60 μm/px」と表示されます。
...
> **キーボード操作**: Method テーブル上で **↑/↓ キー** を押すと前後の化合物に focus が移動します(Compound モードなら全切片に即時反映)。
...
- 各切片パネル左上のラベルにも `Section 1 · 50 μm/px` の形で同じ pixel pitch が併記されます。
